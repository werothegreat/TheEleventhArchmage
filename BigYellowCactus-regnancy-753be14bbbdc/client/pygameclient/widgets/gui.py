...
def createButtonStyle(font, fontcolor, surface, *spaces):
    ...
    style = {'font': font, 'font-color': fontcolor, 'left-normal': surface.subsurface(Rect(0,
             0, spaces[0], h)), 'middle-normal': surface.subsurface(Rect(spaces[0],
             0, spaces[1], h)), 'right-normal': surface.subsurface(Rect(sum(spaces[:2]),
             0, spaces[2], h))}

    if len(spaces) >= 6:
        style['do-over'] = True
        style['left-over'] = surface.subsurface(Rect(sum(spaces[:3]), 0,
                spaces[3], h))
        style['middle-over'] = surface.subsurface(Rect(sum(spaces[:4]),
                0, spaces[4], h))
        style['right-over'] = surface.subsurface(Rect(sum(spaces[:5]), 0,
                spaces[5], h))
    else:
        style['do-over'] = False
    ...
    if len(spaces) >= 9:
        style['do-down'] = True
        style['left-down'] = surface.subsurface(Rect(sum(spaces[:6]), 0,
                spaces[6], h))
        style['middle-down'] = surface.subsurface(Rect(sum(spaces[:7]),
                0, spaces[7], h))
        style['right-down'] = surface.subsurface(Rect(sum(spaces[:8]), 0,
                spaces[8], h))
    else:
        style['do-down'] = False
    ...
    if len(spaces) == 12:
        ...
        style['left-disabled'] = surface.subsurface(Rect(sum(spaces[:9]),
                0, spaces[9], h))
        style['middle-disabled'] = surface.subsurface(Rect(sum(spaces[:10]),
                0, spaces[10], h))
        style['right-disabled'] = surface.subsurface(Rect(sum(spaces[:11]),
                0, spaces[11], h))
    else:
...
def createImageButtonStyle(image, buttonWidth):
    ...
    if image.get_width() >= buttonWidth * 2:
        style['do-over'] = True
        style['image-over'] = image.subsurface(Rect(buttonWidth, 0,
                buttonWidth, h))
    else:
        style['do-over'] = False
    ...
    if image.get_width() >= buttonWidth * 3:
        style['do-down'] = True
        style['image-down'] = image.subsurface(Rect(buttonWidth * 2, 0,
                buttonWidth, h))
    else:
        style['do-down'] = False
    ...
    if image.get_width() >= buttonWidth * 4:
        style['do-disabled'] = True
        style['image-disabled'] = image.subsurface(Rect(buttonWidth * 3,
                0, buttonWidth, h))
    else:
        style['do-disabled'] = False
...
def createCheckBoxStyle(
    font,
    image,
    singleImageWidth,
    font_color=(0, 0, 0),
    disabled_font_color=(150, 150, 150),
    spacing=2,
    borderwidth=0,
    bordercolor=(0, 0, 0),
    autosize=False,
    ...
    style = {
        ...
        'disabled-font-color': disabled_font_color,
        "antialias": antialias,
        'autosize': autosize,
        'wordwrap': wordwrap,
    ...
    style['unchecked-normal'] = image.subsurface(Rect(0, 0,
            singleImageWidth, h))
    style['unchecked-over'] = image.subsurface(Rect(singleImageWidth, 0,
            singleImageWidth, h))
    style['unchecked-down'] = image.subsurface(Rect(singleImageWidth * 2,
            0, singleImageWidth, h))
    style['unchecked-disabled'] = image.subsurface(Rect(singleImageWidth *
            3, 0, singleImageWidth, h))
    style['checked-normal'] = image.subsurface(Rect(singleImageWidth * 4,
            0, singleImageWidth, h))
    style['checked-over'] = image.subsurface(Rect(singleImageWidth * 5,
            0, singleImageWidth, h))
    style['checked-down'] = image.subsurface(Rect(singleImageWidth * 6,
            0, singleImageWidth, h))
    style['checked-disabled'] = image.subsurface(Rect(singleImageWidth *
            7, 0, singleImageWidth, h))

    return style


def createOptionBoxStyle(
    font,
    image,
    singleImageWidth,
    font_color=(0, 0, 0),
    ...
    style['unchecked-down'] = image.subsurface(Rect(singleImageWidth * 2,
            0, singleImageWidth, h))
    style['checked-normal'] = image.subsurface(Rect(singleImageWidth * 3,
            0, singleImageWidth, h))
    style['checked-over'] = image.subsurface(Rect(singleImageWidth * 4,
            0, singleImageWidth, h))
    style['checked-down'] = image.subsurface(Rect(singleImageWidth * 5,
            0, singleImageWidth, h))
    style["disabled"] = image.subsurface(Rect(singleImageWidth * 6, 0,
            singleImageWidth, h))

    return style


def createListBoxStyle(
    font,
    fontcolor,
    fontcolorover,
    fontcolorselected,
    ...
    style = {'font': font, 'font-color-over': fontcolorover,
             'font-color-selected': fontcolorselected, 'bg-color': bgcolor,
             'bg-color-selected': bgcolorselected, 'item-height': itemheight,
             'padding': padding, "antialias": antialias}

    return style


def createComboBoxStyle(font, image, singleImageWidth, font_color=(0, 0,
                        0), borderwidth=0, bordercolor=(0, 0, 0),
                        bgcolor=(255, 255, 255), antialias=True):

    if image.get_width() < singleImageWidth * 4:
        raise GuiException("The image of the CheckBox must contain 7 pictures for the four states (normal, mouse-over and pressed) for each CheckBox value Checked or Unchecked plus one, the last, for the disabled appearence.")

    style = {'font': font, 'font-color': font_color, "antialias": antialias,
             'bg-color': bgcolor, 'border-width': borderwidth,
             'border-color': bordercolor}

    h = image.get_height()

    style['normal'] = image.subsurface(Rect(0, 0, singleImageWidth, h))
    style['over'] = image.subsurface(Rect(singleImageWidth, 0,
            singleImageWidth, h))
    style['down'] = image.subsurface(Rect(singleImageWidth * 2, 0,
            singleImageWidth, h))
    style["disabled"] = image.subsurface(Rect(singleImageWidth * 3, 0,
            singleImageWidth, h))
...
def drawHTiled(frm, lenght, source, dest):
    ...
    dest.blit(source, (frm[0] + k * sw, frm[1]), pygame.Rect(0, 0,
              exceed, source.get_height()))
...
def center(positionHost, sizeHost, sizeClient):
    return ((positionHost[0] + sizeHost[0] / 2) - sizeClient[0] / 2, (positionHost[1] +
            sizeHost[1] / 2) - sizeClient[1] / 2)
...
class Widget(object):
    ...
    def __init__(self, position=(0, 0), size=(100, 20), parent=None,
                 style=None, enabled=True):
        if not parent:
...
class Container:
    ...
    def add(self, widget):
        ...
        self._nextPosition = ((widget.position)[0], (widget.position)[1] +
                              (widget.size)[1])
    ...
    def nextPosition(self, spacing):
        return ((self._nextPosition)[0], (self._nextPosition)[1] +
                spacing)
    ...
    def findTopMost(self, point):
        ...
        if hasattr(topmost, 'findTopMost') and topmost != self:
            return topmost.findTopMost((point[0] - (topmost.position)[0],
                    point[1] - (topmost.position)[1]))
        else:
            return topmost

    def getCenterPoint(self, size):
        return ((self.size)[0] / 2 - size[0] / 2, (self.size)[1] / 2 -
                size[1] / 2)
...
class Desktop(Container):
    ...
    def update(self):
        ...
        if topmost == self:  #Desktop contains the mouse cursor
            if self.onMouseMove:
                self.onMouseMove(pygame.mouse.get_pos(), (pygame.mouse.b1,
                                 pygame.mouse.b2, pygame.mouse.b3))
            ...
            if pygame.mouse.b1 and not (self.mousedown)[0] and self.onMouseDown:
                ...
                self.onClick(pygame.mouse.get_pos(), (pygame.mouse.b1,
                             pygame.mouse.b2, pygame.mouse.b3))
...
class Label(Widget):
    ...
    def __init__(self, position=(0, 0), size=(120, 20), parent=None,
                 style=None, text="Label"):
        ...
        if not style:
            if not defaultLabelStyle:
                import gui
                gui.defaultLabelStyle = {'font-color': (0, 0, 0), 'font': font.Font(None,
                        18), 'autosize': True, "antialias": True,
                        'border-width': False, 'border-color': (0, 0, 0),
                        'wordwrap': False}
    ...
    def refresh(self):
        self.surf = renderText(self.text, (self.style)['font'], (self.style)["antialias"],
                               (self.style)['font-color'], self.size, (self.style)['autosize'],
                               (self.style)['wordwrap'])
...
class Button(Widget):
    ...
    def __init__(self, position=(0, 0), size=(120, 20), parent=None,
                 style=None, enabled=True, text="Button", image=None):
    ...
    def refresh(self):
        ...
        if suffix != self.suffix:
            ...
            if self.image:
                ...
                self.size = (max((self.size)[0], left.get_width() +
                             middle.get_width() + right.get_width()),
                             left.get_height())
            ...
            drawHTiled((left.get_width(), 0), ((self.size)[0] - left.get_width()) -
                       right.get_width(), middle, self.surf)

            self.surf.blit(right, ((self.size)[0] - right.get_width(), 0))

        if not self.image:
            self.textsurf = (self.style)['font'].render(self.text, True,
    ...
    def draw(self, surf):
        if self.visible:
            surf.blit(self.surf, self.position)
            if self.image:
                surf.blit(self.image, center(self.position, self.size,
                          self.image.get_size()))
            else:
                surf.blit(self.textsurf, (((self.position)[0] + (self.size)[0] /
                          2) - self.textsurf.get_size()[0] / 2, ((self.position)[1] +
                          (self.size)[1] / 2) - self.textsurf.get_size()[1] /
                          2))
...
class Window(Widget, Container):
    ...
    def __init__(self, position=None, size=(140, 50), parent=None, style=None,
                 text="Window", closeable=True, shadeable=True):
    ...
    def refresh(self):
        ...
        pos0 = (((self.size)[0] - (self.style)['offset'][0]) - (self.closebutton.size)[0],
                (self.style)['offset'][1])
        if self.closeable:
        ...
        if self.shadeable:
            ...
            self.shadebutton.position = pos0
            if not self.closeable:
                self.shadebutton.position = pos0
            else:
                self.shadebutton.position = ((self.size)[0] - 2 * ((self.style)['offset'][0] +
                        (self.shadebutton.size)[0]), (self.style)['offset'][1])
        ...
        if (self.style)['border-width']:
            draw.rect(self.surf, (self.style)['border-color'], Rect((0,
                      0), self.size), (self.style)['border-width'])

        self.surf.blit((self.style)['font'].render(self.text, True, (self.style)[temp +
                       'font-color']), (self.style)['offset'])
    ...
    def update(self, topmost):
        ...
        if self.moving:
            ...
            self.position = ((self.position)[0] + mx, (self.position)[1] +
                             my)
...
class TextBox(Widget):
    ...
    def refresh(self):
        ...
        if (self.size)[1] < (self.style)['font'].get_ascent() + (self.offset)[1] * \
            2:
            self.size = ((self.size)[0], (self.style)['font'].get_ascent() +
                         (self.offset)[1] * 2)

        self.textsurf = (self.style)['font'].render(self.text, (self.style)["antialias"],
                (self.style)['font-color'])
        ...
        textpos = ((self.offset)[0], (self.size)[1] / 2. - self.textsurf.get_height() /
                   2)
        ...
        if cursorWidth - self._textStartX < (self.size)[0] - (self.offset)[0] * \
            2:
            if cursorx - self._textStartX < 0:
                self._textStartX = max(0, cursorx - ((self.size)[0] - (self.offset)[0] *
                        2))
        else:
            self._textStartX = cursorWidth - ((self.size)[0] - (self.offset)[0] *
                    2)

        self.surf.blit(self.textsurf, textpos, Rect(self._textStartX, 0,
                       (self.size)[0] - (self.offset)[0] * 2, self.textsurf.get_height()))

        if self.hasFocus:
            cursorx -= self._textStartX
            draw.line(self.surf, (255, 255, 255), (cursorx, (self.offset)[1]),
                      (cursorx, (self.size)[1] - (self.offset)[1]))

        if (self.style)['appearence'] == APP_FLAT:
            draw.rect(self.surf, (self.style)['border-color' + suffix],
                      Rect((0, 0), self.size), (self.style)['border-width'])
        else:
            bgcolor = (self.style)['bg-color' + suffix]
            color_dark = colorLuminosity(bgcolor, -50)
            color_light = colorLuminosity(bgcolor, +30)

            draw.line(self.surf, color_dark, (0, 0), ((self.size)[0] - 2,
                      0))  #TOP
            draw.line(self.surf, color_dark, (0, 0), (0, (self.size)[1] -
                      2))  #LEFT
            draw.line(self.surf, color_light, (1, (self.size)[1] - 1), ((self.size)[0],
                      (self.size)[1] - 1))  #LEFT
            draw.line(self.surf, color_light, ((self.size)[0] - 1, 1), ((self.size)[0] -
                      1, (self.size)[1] - 1))  #LEFT
            ...
            for e in events:
                if e.type == pygame.KEYDOWN:
                    if e.key == pygame.K_BACKSPACE:
                        ...
                        idx = self._parent.widgets.index(self)
                        next = (self._parent.widgets)[idx - 1]
                        if isinstance(next, TextBox):
                            self.desktop.focused = next
                            next.refresh()
                            self.refresh()
                    elif e.key == pygame.K_DELETE:

                        self.text = (self.text)[:self.currpos] + (self.text)[self.currpos +
                            1:]
                        ...
                        self.text = (self.text)[:self.currpos] + e.unicode + \
...
class CheckBox(Widget):
    ...
    def __init__(self, position=(0, 0), size=(120, 20), parent=None,
                 style=None, enabled=True, text="CheckBox", value=False):
        if not style:
    ...
    def refresh(self):
        ...
        self.textsurf = renderText(self.text, (self.style)['font'], (self.style)["antialias"],
                                   (self.style)[prefix + 'font-color'],
                                   ((self.size)[0], (self.size)[1]), (self.style)['autosize'],
                                   (self.style)['wordwrap'])

        if (self.style)['autosize']:
            self.size = (self.textsurf.get_width() + (self.style)['spacing'] +
                         (self.style)['checked-normal'].get_width(), max(self.textsurf.get_height(),
                         (self.style)['checked-normal'].get_height()))
...
class ListBox(Widget):
    ...
    def __init__(self, position=(0, 0), size=(120, 20), parent=None,
                 style=None, enabled=True, items=None):
        ...
        if not style:
            ...
            if not gui.defaultListBoxStyle:
                gui.defaultListBoxStyle = {
                    ...
                    'bg-color-selected': (155, 155, 155),
                    'bg-color-over': (215, 215, 215),
                    'border-width': 1,
                    'border-color': (0, 0, 0),
                    'item-height': 18,
                    'padding': 2,
    ...
    def moveDown(self):
        ...
        if (self.selectedIndex + 1) * (self.style)['item-height'] - self._scrolling > \
            (self.size)[1] - (self.style)['padding']:

            self._scrolling += ((self.selectedIndex + 1) * (self.style)['item-height'] -
                                self._scrolling) - ((self.size)[1] - (self.style)['padding'] *
                    2)
    ...
    def moveUp(self):
        ...
        if self.selectedIndex * (self.style)['item-height'] - self._scrolling < \
            (self.style)['padding']:

            self._scrolling += self.selectedIndex * (self.style)['item-height'] - \
                self._scrolling
                ...
                if e.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if e.button == 5 and self.mouseover:  #Wheel-down
                        self._scrolling = min((self.style)['item-height'] *
                                len(self.items) - (self.size)[1] + (self.style)['padding'] *
                                2, self._scrolling + (self.style)['item-height'])
    ...
    def refresh(self):
        ...
        for item in [item.decode('utf-8') for item in self.items]:
            itemWidth = (self.style)['font'].size(item)[0]
        ...
        for i in xrange(len(self.items)):
            ...
            if drawRect:
                draw.rect(self.surf, bgcolor, Rect(0, i * itemheight,
                          self.surf.get_width(), itemheight))

            render = (self.style)['font'].render((self.items)[i].decode('utf-8'),
                    True, fontcolor, bgcolor)
            self.surf.blit(render, (2, (i * itemheight + itemheight / 2) -
                           render.get_height() / 2))
    ...
    def draw(self, surf):
        ...
        surf.blit(self.surf, ((self.position)[0] + self._padding, (self.position)[1] +
                  self._padding), Rect((0, self._scrolling), ((self.size)[0] -
                  self._padding * 2, (self.size)[1] - self._padding * 2)))
